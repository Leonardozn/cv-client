...
import educationFormConfig from "../models/form-source/education";
import educationTableConfig from "../models/table-source/education";
...
export const useEducationController = () => {
	...
	const [formFields, setFormFields] = useState(educationFormConfig);
	const [defaultFields, setDefaultFields] = useState(educationFormConfig);
	const [tableColumns, setTableColumns] = useState(educationTableConfig);
	...
	const loadTableData = useCallback(async (page, size, searchVal) => {
		...
		try {
			...
			triggerPopUp("error", "Error loading educations.");
			setStatus("FAILURE");
		}
	...
	const handleEdit = useCallback(async (item, isView = false) => {
		setStatus("LOADING");
		try {
			const res = await apiMethods.FIND_ONE_EDUCATION.method(item._id || item.id);
			if (res?.content) {
			...
			triggerPopUp("error", "Error fetching education details.");
		} finally {
			setStatus("IDLE");
		}
	}, [triggerPopUp]);
	...
	const handleDelete = useCallback((item) => {
		setConfirm({
			...
			text: `Are you sure you want to delete education "${item.name}"? This action cannot be undone.`,
			onConfirm: async () => {
				setStatus("DELETING");
				try {
					const res = await apiMethods.REMOVE_EDUCATION.method(item._id || item.id);
					triggerPopUp("success", res?.message || "Education deleted successfully.");
					loadTableData(pagination.currentPage, pagination.rowsPerPage, search);
				} catch (error) {
					triggerPopUp("error", error.message || "Error deleting education.");
				} finally {
	...
	const handleSubmit = useCallback(async (formData) => {
		...
		const saveAction = async () => {
			...
			try {
				...
				if (isEditing) {
					await apiMethods.REPLACE_EDUCATION.method(id, payload);
					triggerPopUp("success", "Education updated successfully.");
					handleExternalNotify(true);
				} else {
					const res = await apiMethods.ADD_EDUCATION.method(payload);
					triggerPopUp("success", "Education created successfully.");
				...
				triggerPopUp("error", error.response?.data?.message || error.message || "Error saving education.");
			} finally {
				setConfirm((prev) => ({ ...prev, isOpen: false }));
				setStatus("IDLE");
			}
	...
	useEffect(() => {
		const init = async () => {
			...
			const configWithOptions = await mapDynamicOptions(educationFormConfig, apiMethods);
			setFormFields(configWithOptions);
			setDefaultFields(configWithOptions);

			const resolvedTableCols = await mapTableRenderCells(educationTableConfig, apiMethods);
