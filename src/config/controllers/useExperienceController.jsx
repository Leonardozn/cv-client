...
import experienceFormConfig from "../models/form-source/experience";
import experienceTableConfig from "../models/table-source/experience";
...
export const useExperienceController = () => {
	...
	const [formFields, setFormFields] = useState(experienceFormConfig);
	const [defaultFields, setDefaultFields] = useState(experienceFormConfig);
	const [tableColumns, setTableColumns] = useState(experienceTableConfig);
	...
	const loadTableData = useCallback(async (page, size, searchVal) => {
		...
		try {
			...
			triggerPopUp("error", "Error loading experiences.");
			setStatus("FAILURE");
		}
	...
	const handleEdit = useCallback(async (item, isView = false) => {
		setStatus("LOADING");
		try {
			const res = await apiMethods.FIND_ONE_EXPERIENCE.method(item._id || item.id);
			if (res?.content) {
			...
			triggerPopUp("error", "Error fetching experience details.");
		} finally {
			setStatus("IDLE");
		}
	}, [triggerPopUp]);
	...
	const handleDelete = useCallback((item) => {
		setConfirm({
			...
			text: `Are you sure you want to delete experience "${item.name}"? This action cannot be undone.`,
			onConfirm: async () => {
				setStatus("DELETING");
				try {
					const res = await apiMethods.REMOVE_EXPERIENCE.method(item._id || item.id);
					triggerPopUp("success", res?.message || "Experience deleted successfully.");
					loadTableData(pagination.currentPage, pagination.rowsPerPage, search);
				} catch (error) {
					triggerPopUp("error", error.message || "Error deleting experience.");
				} finally {
	...
	const handleSubmit = useCallback(async (formData) => {
		...
		const saveAction = async () => {
			...
			try {
				...
				if (isEditing) {
					await apiMethods.REPLACE_EXPERIENCE.method(id, payload);
					triggerPopUp("success", "Experience updated successfully.");
					handleExternalNotify(true);
				} else {
					const res = await apiMethods.ADD_EXPERIENCE.method(payload);
					triggerPopUp("success", "Experience created successfully.");
				...
				triggerPopUp("error", error.response?.data?.message || error.message || "Error saving experience.");
			} finally {
				setConfirm((prev) => ({ ...prev, isOpen: false }));
				setStatus("IDLE");
			}
	...
	useEffect(() => {
		const init = async () => {
			...
			const configWithOptions = await mapDynamicOptions(experienceFormConfig, apiMethods);
			setFormFields(configWithOptions);
			setDefaultFields(configWithOptions);

			const resolvedTableCols = await mapTableRenderCells(experienceTableConfig, apiMethods);
