...
import curriculumFormConfig from "../models/form-source/curriculum";
import curriculumTableConfig from "../models/table-source/curriculum";

export const useCurriculumController = () => {
	// ── State ──────────────────────────────────────────────────────────────────
	...
	const [formFields, setFormFields] = useState(curriculumFormConfig);
	const [defaultFields, setDefaultFields] = useState(curriculumFormConfig);
	const [tableColumns, setTableColumns] = useState(curriculumTableConfig);
	...
	const loadTableData = useCallback(async (page, size, searchVal) => {
		...
		try {
			...
			triggerPopUp("error", "Error loading curriculums.");
			setStatus("FAILURE");
		}
	...
	const handleEdit = useCallback(async (item, isView = false) => {
		setStatus("LOADING");
		try {
			const res = await apiMethods.FIND_ONE_CURRICULUM.method(item._id || item.id);
			if (res?.content) {
			...
			triggerPopUp("error", "Error fetching curriculum details.");
		} finally {
			setStatus("IDLE");
		}
	}, [triggerPopUp]);
	...
	const handleDelete = useCallback((item) => {
		setConfirm({
			...
			text: `Are you sure you want to delete curriculum "${item.name}"? This action cannot be undone.`,
			onConfirm: async () => {
				setStatus("DELETING");
				try {
					const res = await apiMethods.REMOVE_CURRICULUM.method(item._id || item.id);
					triggerPopUp("success", res?.message || "Curriculum deleted successfully.");
					loadTableData(pagination.currentPage, pagination.rowsPerPage, search);
				} catch (error) {
					triggerPopUp("error", error.message || "Error deleting curriculum.");
				} finally {
	...
	const handleSubmit = useCallback(async (formData) => {
		...
		const saveAction = async () => {
			...
			try {
				...
				if (isEditing) {
					await apiMethods.REPLACE_CURRICULUM.method(id, payload);
					triggerPopUp("success", "Curriculum updated successfully.");
					handleExternalNotify(true);
				} else {
					const res = await apiMethods.ADD_CURRICULUM.method(payload);
					triggerPopUp("success", "Curriculum created successfully.");
				...
				triggerPopUp("error", error.response?.data?.message || error.message || "Error saving curriculum.");
			} finally {
				setConfirm((prev) => ({ ...prev, isOpen: false }));
				setStatus("IDLE");
			}
	...
	useEffect(() => {
		const init = async () => {
			...
			const configWithOptions = await mapDynamicOptions(curriculumFormConfig, apiMethods);
			setFormFields(configWithOptions);
			setDefaultFields(configWithOptions);

			const resolvedTableCols = await mapTableRenderCells(curriculumTableConfig, apiMethods);
