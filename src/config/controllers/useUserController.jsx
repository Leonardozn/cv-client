...
import userFormConfig from "../models/form-source/user";
import userTableConfig from "../models/table-source/user";
...
export const useUserController = () => {
	...
	const [formFields, setFormFields] = useState(userFormConfig);
	const [defaultFields, setDefaultFields] = useState(userFormConfig);
	const [tableColumns, setTableColumns] = useState(userTableConfig);
	...
	const loadTableData = useCallback(async (page, size, searchVal) => {
		...
		try {
			...
			triggerPopUp("error", "Error loading users.");
			setStatus("FAILURE");
		}
	...
	const handleEdit = useCallback(async (item, isView = false) => {
		setStatus("LOADING");
		try {
			const res = await apiMethods.FIND_ONE_USER.method(item._id || item.id);
			if (res?.content) {
			...
			triggerPopUp("error", "Error fetching user details.");
		} finally {
			setStatus("IDLE");
		}
	}, [triggerPopUp]);
	...
	const handleDelete = useCallback((item) => {
		setConfirm({
			isOpen: true,
			title: "Delete User",
			text: `Are you sure you want to delete user "${item.name}"? This action cannot be undone.`,
			onConfirm: async () => {
				setStatus("DELETING");
				try {
					const res = await apiMethods.REMOVE_USER.method(item._id || item.id);
					triggerPopUp("success", res?.message || "User deleted successfully.");
					loadTableData(pagination.currentPage, pagination.rowsPerPage, search);
				} catch (error) {
					triggerPopUp("error", error.message || "Error deleting user.");
				} finally {
	...
	const handleSubmit = useCallback(async (formData) => {
		...
		const saveAction = async () => {
			...
			try {
				...
				if (isEditing) {
					await apiMethods.REPLACE_USER.method(id, payload);
					triggerPopUp("success", "User updated successfully.");
					handleExternalNotify(true);
				} else {
					const res = await apiMethods.ADD_USER.method(payload);
					triggerPopUp("success", "User created successfully.");
				...
				triggerPopUp("error", error.response?.data?.message || error.message || "Error saving user.");
			} finally {
				setConfirm((prev) => ({ ...prev, isOpen: false }));
				setStatus("IDLE");
			}
	...
	useEffect(() => {
		const init = async () => {
			...
			const configWithOptions = await mapDynamicOptions(userFormConfig, apiMethods);
			setFormFields(configWithOptions);
			setDefaultFields(configWithOptions);

			const resolvedTableCols = await mapTableRenderCells(userTableConfig, apiMethods);
