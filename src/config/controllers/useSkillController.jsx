...
import skillFormConfig from "../models/form-source/skill";
import skillTableConfig from "../models/table-source/skill";
...
export const useSkillController = () => {
	...
	const [formFields, setFormFields] = useState(skillFormConfig);
	const [defaultFields, setDefaultFields] = useState(skillFormConfig);
	const [tableColumns, setTableColumns] = useState(skillTableConfig);
	...
	const loadTableData = useCallback(async (page, size, searchVal) => {
		...
		try {
			...
			triggerPopUp("error", "Error loading skills.");
			setStatus("FAILURE");
		}
	...
	const handleEdit = useCallback(async (item, isView = false) => {
		setStatus("LOADING");
		try {
			const res = await apiMethods.FIND_ONE_SKILL.method(item._id || item.id);
			if (res?.content) {
			...
			triggerPopUp("error", "Error fetching skill details.");
		} finally {
			setStatus("IDLE");
		}
	}, [triggerPopUp]);
	...
	const handleDelete = useCallback((item) => {
		setConfirm({
			isOpen: true,
			title: "Delete Skill",
			text: `Are you sure you want to delete skill "${item.name}"? This action cannot be undone.`,
			onConfirm: async () => {
				setStatus("DELETING");
				try {
					const res = await apiMethods.REMOVE_SKILL.method(item._id || item.id);
					triggerPopUp("success", res?.message || "Skill deleted successfully.");
					loadTableData(pagination.currentPage, pagination.rowsPerPage, search);
				} catch (error) {
					triggerPopUp("error", error.message || "Error deleting skill.");
				} finally {
	...
	const handleSubmit = useCallback(async (formData) => {
		...
		const saveAction = async () => {
			...
			try {
				...
				if (isEditing) {
					await apiMethods.REPLACE_SKILL.method(id, payload);
					triggerPopUp("success", "Skill updated successfully.");
					handleExternalNotify(true);
				} else {
					const res = await apiMethods.ADD_SKILL.method(payload);
					triggerPopUp("success", "Skill created successfully.");
				...
				triggerPopUp("error", error.response?.data?.message || error.message || "Error saving skill.");
			} finally {
				setConfirm((prev) => ({ ...prev, isOpen: false }));
				setStatus("IDLE");
			}
	...
	useEffect(() => {
		const init = async () => {
			...
			const configWithOptions = await mapDynamicOptions(skillFormConfig, apiMethods);
			setFormFields(configWithOptions);
			setDefaultFields(configWithOptions);

			const resolvedTableCols = await mapTableRenderCells(skillTableConfig, apiMethods);
