...
import certificateFormConfig from "../models/form-source/certificate";
import certificateTableConfig from "../models/table-source/certificate";
...
export const useCertificateController = () => {
	...
	const [formFields, setFormFields] = useState(certificateFormConfig);
	const [defaultFields, setDefaultFields] = useState(certificateFormConfig);
	const [tableColumns, setTableColumns] = useState(certificateTableConfig);
	...
	const loadTableData = useCallback(async (page, size, searchVal) => {
		...
		try {
			...
			triggerPopUp("error", "Error loading certificates.");
			setStatus("FAILURE");
		}
	...
	const handleEdit = useCallback(async (item, isView = false) => {
		setStatus("LOADING");
		try {
			const res = await apiMethods.FIND_ONE_CERTIFICATE.method(item._id || item.id);
			if (res?.content) {
			...
			triggerPopUp("error", "Error fetching certificate details.");
		} finally {
			setStatus("IDLE");
		}
	}, [triggerPopUp]);
	...
	const handleDelete = useCallback((item) => {
		setConfirm({
			...
			text: `Are you sure you want to delete certificate "${item.name}"? This action cannot be undone.`,
			onConfirm: async () => {
				setStatus("DELETING");
				try {
					const res = await apiMethods.REMOVE_CERTIFICATE.method(item._id || item.id);
					triggerPopUp("success", res?.message || "Certificate deleted successfully.");
					loadTableData(pagination.currentPage, pagination.rowsPerPage, search);
				} catch (error) {
					triggerPopUp("error", error.message || "Error deleting certificate.");
				} finally {
	...
	const handleSubmit = useCallback(async (formData) => {
		...
		const saveAction = async () => {
			...
			try {
				...
				if (isEditing) {
					await apiMethods.REPLACE_CERTIFICATE.method(id, payload);
					triggerPopUp("success", "Certificate updated successfully.");
					handleExternalNotify(true);
				} else {
					const res = await apiMethods.ADD_CERTIFICATE.method(payload);
					triggerPopUp("success", "Certificate created successfully.");
				...
				triggerPopUp("error", error.response?.data?.message || error.message || "Error saving certificate.");
			} finally {
				setConfirm((prev) => ({ ...prev, isOpen: false }));
				setStatus("IDLE");
			}
	...
	useEffect(() => {
		const init = async () => {
			...
			const configWithOptions = await mapDynamicOptions(certificateFormConfig, apiMethods);
			setFormFields(configWithOptions);
			setDefaultFields(configWithOptions);

			const resolvedTableCols = await mapTableRenderCells(certificateTableConfig, apiMethods);
