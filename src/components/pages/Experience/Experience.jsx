...
import { useExperienceController } from "../../../config/controllers/useExperienceController";
...
const Experience = () => {
	...
	return (
		<div className="experience-page">
			<h1 className="experience-title">Experience Management</h1>

			<Sheet className="experience-actions">
				<div className="experience-actions-left">
					{isLoading && <Spinner style={{ "--spinner-size": "2rem" }} />}
				</div>
				<div className="experience-actions-btn">
					<Button 
						text="New Experience" 
						type="primary" 
			...
			<Sheet className="experience-content">
				{data.length === 0 && !search.value && !isLoading ? (
					<p className="experience-empty-state">
						No records found. Use the "New Experience" button to add one.
					</p>
				) : (
					<DataTable
						columns={[
			...
			<Modal
				...
				title={`${modal.mode === 'VIEW' ? 'View' : modal.mode === 'EDIT' ? 'Edit' : 'New'} Experience`}
				size="md"
			>
				<Form
					...
					submitText={modal.mode === 'EDIT' ? 'Save changes' : 'Add Experience'}
