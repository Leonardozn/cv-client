...
import { useUserController } from "../../../config/controllers/useUserController";
...
const User = () => {
	...
	return (
		<div className="user-page">
			...
			<Sheet className="user-content">
				{data.length === 0 && !search.value && !isLoading ? (
					<p className="user-empty-state">
						No records found. Use the "New User" button to add one.
					</p>
				) : (
					<DataTable
						columns={[
			...
			<Modal
				...
				title={`${modal.mode === 'VIEW' ? 'View' : modal.mode === 'EDIT' ? 'Edit' : 'New'} User`}
				size="md"
			>
				<Form
					...
					submitText={modal.mode === 'EDIT' ? 'Save changes' : 'Add User'}
