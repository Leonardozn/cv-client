...
import { useSkillController } from "../../../config/controllers/useSkillController";
...
const Skill = () => {
	...
	return (
		<div className="skill-page">
			<h1 className="skill-title">Skill Management</h1>

			<Sheet className="skill-actions">
				<div className="skill-actions-left">
					{isLoading && <Spinner style={{ "--spinner-size": "2rem" }} />}
				</div>
				<div className="skill-actions-btn">
					<Button 
						text="New Skill" 
						type="primary" 
			...
			<Sheet className="skill-content">
				{data.length === 0 && !search.value && !isLoading ? (
					<p className="skill-empty-state">
						No records found. Use the "New Skill" button to add one.
					</p>
				) : (
					<DataTable
						columns={[
			...
			<Modal
				...
				title={`${modal.mode === 'VIEW' ? 'View' : modal.mode === 'EDIT' ? 'Edit' : 'New'} Skill`}
				size="md"
			>
				<Form
					...
					submitText={modal.mode === 'EDIT' ? 'Save changes' : 'Add Skill'}
