...
import { useEducationController } from "../../../config/controllers/useEducationController";
...
const Education = () => {
	...
	return (
		<div className="education-page">
			<h1 className="education-title">Education Management</h1>

			<Sheet className="education-actions">
				<div className="education-actions-left">
					{isLoading && <Spinner style={{ "--spinner-size": "2rem" }} />}
				</div>
				<div className="education-actions-btn">
					<Button 
						text="New Education" 
						type="primary" 
			...
			<Sheet className="education-content">
				{data.length === 0 && !search.value && !isLoading ? (
					<p className="education-empty-state">
						No records found. Use the "New Education" button to add one.
					</p>
				) : (
					<DataTable
						columns={[
			...
			<Modal
				...
				title={`${modal.mode === 'VIEW' ? 'View' : modal.mode === 'EDIT' ? 'Edit' : 'New'} Education`}
				size="md"
			>
				<Form
					...
					submitText={modal.mode === 'EDIT' ? 'Save changes' : 'Add Education'}
